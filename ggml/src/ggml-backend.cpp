...
static bool ggml_backend_sched_prefetch_init(ggml_backend_sched_t sched, ggml_backend_t split_backend, size_t size) {
    if (sched->prefetch_backend == NULL) {
        ...
        // the prefetch path re-uploads experts through a second backend instance whose
        // independent stream races with the split backend on HIP/RDNA3 (illegal memory
        // access). The MoE expert cache replaces the H2D-upload overlap there, so keep
        // the prefetch disabled on ROCm.
