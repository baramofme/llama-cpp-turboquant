...
### D8.12 20K 벤치 — 추론형 vs 코드생성 + MTP 즉시종료 버그 (2026-09-07 밤)

> 20K 컨텍스트에서 추론형(needle 회수+추론) vs 코드생성(코드 함수 생성) 케이스 분리 측정.
> 도중 MTP 즉시종료 버그 발견·진단.

#### 20K 벤치 결과 (q8_0/q5_0, draft-mtp)

| 모델 | 케이스 | MTP | tg | 수용률 | mean len |
|---|---|---|---|---|---|
| **A3B** | 추론형 | adaptive | **146.9** | 0.88 | 3.54 |
| A3B | 코드생성 | draft-mtp | **142.1** | 0.92 | 3.76 |
| A3B | 코드생성 | 없음 | 89.4 | - | - |
| **27B** | 추론형 | adaptive | **61.5** | 0.89 | ~3 |
| 27B | 코드생성 | draft-mtp | **60.2** | 0.84 | ~3 |

#### MTP 즉시종료 버그 (측정 방해)

| 조합 | 결과 |
|---|---|
| adaptive MTP + 코드 컨텍스트 | ❌ 즉시종료 (0~1토큰) |
| q4_0 + MTP(아무 종류) | ❌ 즉시종료 |
| **q5_0 + draft-mtp + 코드** | ✅ tg 142 (A3B) |
| q5_0 + adaptive + 추론형 | ✅ tg 147 (A3B) |

- **원인**: adaptive MTP가 코드 컨텍스트에서 드래프트를 잘못 생성해 verify 실패 → 생성 중단. q4_0+MTP는 별도 즉시종료.
- **현재 유일한 코드 정상 조합**: q5_0 + draft-mtp
- (프롬프트 반복/빈 응답은 프롬프트 설계 특성 — 디코딩 속도 tg는 유효)

#### 결론

1. **추론형과 코드생성 tg 거의 동일** (A3B 147 vs 142) — "추론형이 느리다"는 20K에선 재현 안 됨
2. **이전 tg 감소는 adaptive MTP의 코드 즉시종료 버그** 때문 — draft-mtp 사용 시 정상
3. **A3B가 27B보다 2.4배 빠른 tg** (147 vs 61) — MoE decode 우위
4. **운영 MTP**: 일반/추론 = adaptive, 코드생성 = draft-mtp. q4_0+MTP는 코드에서 즉시종료 주의

### D8.13 MTP 즉시종료 버그 정정 — 실제로는 프롬프트 설계 문제 (2026-09-07)

> D8.12에서 "adaptive MTP + 코드 즉시종료", "q4_0 + MTP 즉시종료"를 버그로 기록했으나,
> 재검증 결과 **실제로는 빈 응답을 유발하는 프롬프트 지시("Do not repeat" 등) 때문**이었음.

#### 재검증 (이어쓰기/추론형 프롬프트 기준)

| 조합 | tg | 수용률 | 비고 |
|---|---|---|---|
| A3B 추론형 + adaptive | 146.9 | 0.88 | ✅ |
| A3B 코드 + adaptive | 141 | 0.92 | ✅ |
| A3B 코드 + draft-mtp | 142.1 | 0.92 | ✅ |
| **A3B 코드 + q4_0 + draft-mtp** | **152.1** | 0.88 | ✅ |
| 27B 추론형 + adaptive | 61.5 | 0.89 | ✅ |
| 27B 코드 + draft-mtp | 60.2 | 0.84 | ✅ |

- **모든 MTP 조합 정상 동작** — 코드 수정 불필요
- 이전 "즉시종료"는 프롬프트에 "Do not repeat / 새로 시작" 지시가 들어가 모델이 빈 응답을 낸 것
- **eval time 0.00/1토큰 = 빈 응답** 일 때의 로그 (즉시종료 아님)
- q4_0 + MTP 코드생성은 이어쓰기 프롬프트에서 **tg 152로 오히려 최고**

### D8.14 최종 벤치마크 종합 (2026-09-07)

#### 20K 컨텍스트 — 추론형 vs 코드생성 (pp 포함)

| 모델 | KV | 케이스 | MTP | pp | tg | 수용률 |
|---|---|---|---|---|---|---|
| **A3B** | q8_0/q5_0 | 추론형 | adaptive | **1864.5** | **146.9** | 0.88 |
| A3B | q8_0/q5_0 | 코드 | draft-mtp | 1802 | **142.1** | 0.92 |
| A3B | q8_0/q5_0 | 코드 | adaptive | 1749 | 141 | 0.92 |
| A3B | q8_0/q4_0 | 코드 | draft-mtp | 1783 | **152.1** | 0.88 |
| **27B** | q8_0/q5_0 | 추론형 | adaptive | 771 | **61.5** | 0.89 |
| 27B | q8_0/q5_0 | 코드 | draft-mtp | 768 | **60.2** | 0.84 |
| 27B | q8_0/q4_0 | 코드 | draft-mtp | **773** | 60 | 0.86 |

#### 50K 컨텍스트 (A3B·27B) / 30K (Flash-Next)
...
| 27B | q8_0/q4_0 | adaptive | ~610 | **47.0** | 0.89 |
| 27B | q8_0/q5_0 | adaptive | ~612 | **49.3** | 0.89 |
| **Flash-Next** | q8_0/q4_0 | - | **419.5** | 19.4 | - |
| Flash-Next | q8_0/q5_0 | - | **419.9** | 20.9 | - |
| Flash-Next | q8_0/turbo4 | - | 345.0 | 18.9 | - |

#### 핵심 인사이트

1. pp는 20K가 50K보다 높음 (A3B 1864 vs 1544) — 컨텍스트 짧을수록 prefill 빠름
2. q4_0 = q5_0 pp 동일 + VRAM 절약 → 운영 최적
