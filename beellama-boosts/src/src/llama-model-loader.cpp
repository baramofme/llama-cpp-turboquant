    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        if (use_mmap) {
            ...
            if (buf_mmap && cur->data == nullptr) {
                ...
                // destination is host memory (e.g. a pinned ROCm_Host buffer selected by -ot).
                // copying from the mapping would fault in every page one by one at queue depth 1,
                // and with --numa distribute the mapping carries POSIX_MADV_RANDOM which disables
                // readahead. a plain read gets full readahead and is much faster.
